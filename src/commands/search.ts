...
export async function searchCommand(
  ...
  try {
    ...
    if (type) {
      console.log(Formatter.formatSuccess(`Found ${results.length} ${type}(s)`));
    } else {
    ...
    if (results.length === 0) {
      console.log(Formatter.formatWarning('Try different keywords or check your spelling'));
    }
    ...
    if (error.name === 'YtSearchError') {
      ...
      if (error.metadata) {
        console.log(Formatter.formatWarning(`Additional info: ${JSON.stringify(error.metadata)}`));
      }
    } else {
